...
export function FloatWindow() {
  ...
  const kindLabel = (b: TimeBlock) => (b.kind === 'focus' ? t.focus : t.leisure);
  // Same mapping as the beui demo: swipe right reveals the left rail (Done), swipe left reveals the right rail (Later / Trash).
  const doneAction: SwipeAction = { id: 'confirm', label: t.done, icon: <Check className="h-4 w-4" />, tone: 'success' };
  const skipAction: SwipeAction = { id: 'skip', label: t.skip, icon: <Clock3 className="h-4 w-4" />, tone: 'warning' };
  const trashAction: SwipeAction = { id: 'trash', label: t.trash, icon: <Trash2 className="h-4 w-4" />, tone: 'danger' };
  const resetAction: SwipeAction = { id: 'reset', label: t.reset, icon: <RotateCcw className="h-4 w-4" />, tone: 'neutral' };
  const now = useNow();
  ...
  const items: SwipeableListItem[] = rows.map(({ b, st, running, progress, status, actionable }) => ({
    ...
    description: `${fmtHm(b.startMin!)}–${fmtHm(b.endMin!)} · ${kindLabel(b)}${
      running ? ` · ${t.inProgress}` : st === 'pending' ? ` · ${t.due}` : st === 'planned' ? ` · ${t.plannedTag}` : ''
    }`,
    // meta like the demo: a time on the right; "Now" while running, "Due" once it has ended unconfirmed
    meta:
      running && progress !== undefined ? (
        <span className="text-amber-400 tabular-nums">{t.now} · {progress}%</span>
      ) : st === 'pending' ? (
  ...
  const tally = rows.length === 0 ? t.nothingPlannedYet : t.tally(done, open);
    ...
    <div className="flex h-screen w-full items-stretch justify-center">
      ...
      <div className="flex w-full max-w-sm flex-col rounded-[2rem] border border-border bg-background p-3 shadow-2xl">
        <div className="mb-3 flex items-center justify-between px-1" data-tauri-drag-region>
          <div data-tauri-drag-region>
            <p className="text-sm font-semibold text-foreground" data-tauri-drag-region>
              {t.today} · <span className="tabular-nums">{clock}</span>
            </p>
        ...
        <div className="scrollbar-hide min-h-0 flex-1 overflow-y-auto">
          {items.length === 0 ? (
            <p className="px-1 py-6 text-xs text-muted-foreground">{t.planInMain}</p>
          ) : (
            <SwipeableList
              items={items}
        ...
        >
          <AnimatePresence mode="wait" initial={false}>
            {undo ? (
              ...
              >
                {t.deleted(undo.block.note || kindLabel(undo.block))}
              ...
              <motion.span key="hint" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} data-tauri-drag-region>
                {rows.some((r) => r.actionable) ? t.swipeHint : t.nPlannedFooter(rows.length)}
              </motion.span>
            )}
          </AnimatePresence>
          <span data-tauri-drag-region>
            {t.focus} {fmtMinutes(focusMin)} · {t.leisure} {fmtMinutes(funMin)}
          </span>
        </div>
      </div>
    </div>
